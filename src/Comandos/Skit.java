...
public class Skit implements Listener, CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("skit")) {
            ...
            if (args.length == 0) {
                p.sendMessage(String.valueOf(Main.prefix) + " >>§7 Sintaxe correta: §c/skit criar|aplicar <nome>|<distancia>");
                return true;
            }
            if (args[0].equalsIgnoreCase("criar")) {
                ...
                if (args.length <= 2) {
                    p.sendMessage(String.valueOf(Main.prefix) + " >>§7 Uso correto: §c/skit aplicar <nome> <distancia>");
                    return true;
                }
                final String name = args[1];
                ...
                if (this.isInt(args[2])) {
                    ...
                    Bukkit.getServer().broadcastMessage(String.valueOf(Main.prefix) + " >>§7 Kit§c " + name + "§7 aplicado para jogadores em um raio de§c " + numero + "§7 blocos do staffer " + p.getName());
                    p.sendMessage(String.valueOf(Main.prefix) + " >>§7 Kit§c " + name + "§7 aplicado para jogadores em um raio de§c " + numero + "§7 blocos de voc\u00ea.");
                    return true;
                }
                return true;
            }
