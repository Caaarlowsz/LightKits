...
public class AdminCommand implements Listener, CommandExecutor
{
    ...
    public void aoInteragir(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if (AdminCommand.admin.contains(p.getName()) && (e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.RIGHT_CLICK_AIR) && p.getItemInHand().getType() == Material.MAGMA_CREAM) {
            p.chat("/admin");
    ...
    public void aoInteragi2(final PlayerInteractEvent e) {
        final Player p = e.getPlayer();
        if (AdminCommand.admin.contains(p.getName()) && (e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.RIGHT_CLICK_AIR) && p.getItemInHand().getType() == Material.STAINED_GLASS_PANE) {
            p.chat("/admin");
        }
    }
