        ...
        if (cmd.getName().equalsIgnoreCase("build")) {
            if (p.hasPermission("cmd.build")) {
                if (args.length == 0) {
                    ...
                    if (t == null) {
                        p.sendMessage(String.valueOf(String.valueOf(Main.prefix)) + " §7» §cEste Player Est\u00e1 Offline");
                        return true;
                    }
                    if (!BuildCommand.embuild.contains(t)) {
                        BuildCommand.embuild.add(t);
                        p.sendMessage(String.valueOf(String.valueOf(Main.prefix)) + " §7» §7Build §a§lATIVADO§7 Para " + t.getName());
                    }
                    else {
                        BuildCommand.embuild.remove(t);
                        p.sendMessage(String.valueOf(String.valueOf(Main.prefix)) + " §7» §7Build §c§lDESATIVADO§7 Para " + t.getName());
                    }
                }
            }
            else {
                p.sendMessage(String.valueOf(String.valueOf(Main.prefix)) + " §7» Desculpe, Mas voce precisa do grupo §5Moderador§7 ou Superior para executar isto.");
            }
        }
        return false;
    }
