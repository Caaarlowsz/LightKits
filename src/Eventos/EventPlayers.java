...
public class EventPlayers implements Listener {
	...
	public void onColorandChangeChat(final PlayerChatEvent e) {
		...
		if (!p.hasPermission("ver.report") && (e.getMessage().contains(".net") || e.getMessage().contains(".com")
				|| e.getMessage().contains(".crafted") || e.getMessage().contains(".com.br")
	...
	public void onClickItem5(final PlayerInteractEvent e) {
		final Player p = e.getPlayer();
		if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK)
				&& e.getPlayer().getItemInHand().getType() == Material.JUKEBOX) {
			e.setCancelled(true);
			p.playSound(p.getLocation(), Sound.CHEST_OPEN, 5.0f, 5.0f);
