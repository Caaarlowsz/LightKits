...
public class LojaInventory implements Listener, CommandExecutor
{
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        final Player p = (Player)e.getWhoClicked();
        if (e.getInventory().getTitle().equalsIgnoreCase("§8Loja") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
            if (e.getCurrentItem().getType() == Material.BOOK) {
                e.setCancelled(true);
                p.closeInventory();
                p.sendMessage(String.valueOf(String.valueOf(Main.prefix)) + " §6» §7Acesse nosso Site §6» " + Main.site);
            }
        }
        if (e.getInventory().getTitle().equalsIgnoreCase("§8Loja") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
